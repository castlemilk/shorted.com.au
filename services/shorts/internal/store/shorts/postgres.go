...
func (s *postgresStore) GetTopShorts(period string, limit int32, offset int32, summaryOnly bool, productCodes ...string) ([]*stocksv1alpha1.TimeSeriesData, int, error) {
	return FetchTimeSeriesData(s.db, int(limit), int(offset), period, summaryOnly, productCodes...)
}
...
func (s *postgresStore) GetMarketByDate(date string, limit, offset int32, includeZero, ordinaryOnly bool) ([]*stocksv1alpha1.Stock, int, error) {
	...
	// the rows beside it. Reported twice: as #565 (totalCount 731 next to 689
	// rows) and again as #577 after this fix was accidentally reverted —
	// limit 100 returning 99, limit 500 returning 497, and six names of a
	// 553-row universe unreachable at any page size.
	//
