...
async def generate_stock_data(stock_code: str, base_price: float, num_days: int = 30):
    ...
    for i in range(num_days):
        ...
        high_price = max(open_price, close_price) + abs(daily_change) * random.uniform(0, 0.8)
        low_price = min(open_price, close_price) - abs(daily_change) * random.uniform(0, 0.8)
...
async def add_missing_stocks():
    ...
    try:
        ...
        existing_stocks = await conn.fetch("SELECT DISTINCT stock_code FROM stock_prices")
        ...
        stocks_to_add = {code: price for code, price in MISSING_STOCKS.items() 
        ...
        print(f"➕ Adding {len(stocks_to_add)} missing stocks: {list(stocks_to_add.keys())}")
        ...
        final_stocks = await conn.fetch("SELECT DISTINCT stock_code FROM stock_prices ORDER BY stock_code")
        
        print(f"\n🎉 Successfully added {total_records} new records!")
        print(f"📊 Total records in database: {final_count}")
        print(f"📈 All stocks now in database: {[row['stock_code'] for row in final_stocks]}")
        ...
        if wow_sample:
            ...
            for row in wow_sample:
                print(f"{row['date']} | ${row['open']:<6} | ${row['high']:<6} | ${row['low']:<6} | ${row['close']:<6} | {row['volume']:>7,}")
