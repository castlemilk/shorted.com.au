...
async def populate_all_stocks():
    ...
    print(f"🚀 Populating {len(ASX_STOCKS)} ASX stocks with 10 years of real historical data")
    ...
    try:
        ...
        print(f"📅 Overall date range: {overall_range['earliest']} to {overall_range['latest']}")
        ...
        for row in final_stats:
            if row['earliest'] and row['latest']:
                ...
                print(f"{row['stock_code']:<5} | {row['records']:>7,} | {'N/A':<10} | {'N/A':<10} | {'N/A':>4}")
        
        # Show total years of data available
        total_data_years = sum((row['latest'] - row['earliest']).days / 365.25 
                              for row in final_stats 
                              if row['earliest'] and row['latest'])
        
        print(f"\n📊 Total years of historical data: {total_data_years:.1f} years")
        print(f"📈 Average years per stock: {total_data_years / len(final_stats):.1f} years")
