...
async def update_wow_with_real_data():
    ...
    print(
        f"✅ Fetched {len(hist)} records from {hist.index[0].date()} to {hist.index[-1].date()}"
    )
    ...
    try:
        ...
        existing = await conn.fetchval(
            "SELECT COUNT(*) FROM stock_prices WHERE stock_code = 'WOW'"
        )
        ...
        print(
            f"💰 Price range: ${wow_stats['min_price']:.2f} - ${wow_stats['max_price']:.2f}"
        )
        ...
        for row in sample:
            print(
                f"{row['date']} | ${row['open']:<6.2f} | ${row['high']:<6.2f} | ${row['low']:<6.2f} | ${row['close']:<6.2f} | {row['volume']:>7,}"
            )
