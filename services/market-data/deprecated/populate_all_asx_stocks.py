...
CHECKPOINT_FILE = Path("populate_progress.json")
...
def get_remaining_stocks(all_stocks, checkpoint):
    """Get stocks that haven't been processed yet"""
    completed = set(checkpoint["completed_stocks"])
    failed = set(checkpoint["failed_stocks"])
    processed = completed | failed

    remaining = {k: v for k, v in all_stocks.items() if k not in processed}
    return remaining
...
async def populate_all_stocks(batch_stocks=None, checkpoint=None):
    """Populate all stocks with 10 years of real data"""
    stocks_to_process = batch_stocks if batch_stocks else ASX_STOCKS

    print(
        f"🚀 Populating {len(stocks_to_process)} ASX stocks with 10 years of real historical data"
    )
    ...
    try:
        ...
        for i, (stock_code, yf_ticker) in enumerate(stocks_to_process.items(), 1):
            ...
            try:
                inserted, status = await update_stock(conn, stock_code, yf_ticker)

                if status == "success" and inserted > 0:
                    total_inserted += inserted
                    successful_stocks += 1
                    if checkpoint:
                        checkpoint["completed_stocks"].append(stock_code)
                        checkpoint["total_records_inserted"] += inserted
                        save_checkpoint(checkpoint)
                elif status == "skipped":
                    successful_stocks += 1
                    if checkpoint and stock_code not in checkpoint["completed_stocks"]:
                        checkpoint["completed_stocks"].append(stock_code)
                        save_checkpoint(checkpoint)
                else:
                    failed_stocks.append(stock_code)
                    if checkpoint:
                        checkpoint["failed_stocks"].append(stock_code)
                        save_checkpoint(checkpoint)
            except Exception as e:
        ...
        print(
            f"✅ Successfully processed: {successful_stocks}/{len(stocks_to_process)} stocks"
        )
...
async def main():
    """Main function to run in batches with checkpointing"""
    # Load checkpoint
    ...
    # Get remaining stocks to process
    remaining_stocks = get_remaining_stocks(ASX_STOCKS, checkpoint)

    if not remaining_stocks:
        print("🎉 All stocks have been processed!")
        print(f"📊 Total records inserted: {checkpoint['total_records_inserted']:,}")
        print(f"✅ Completed stocks: {len(checkpoint['completed_stocks'])}")
        print(f"❌ Failed stocks: {len(checkpoint['failed_stocks'])}")
        if checkpoint["failed_stocks"]:
            print(f"Failed: {', '.join(checkpoint['failed_stocks'])}")
        return

    print(f"📋 Progress Summary:")
    print(f"  ✅ Completed: {len(checkpoint['completed_stocks'])} stocks")
    print(f"  ❌ Failed: {len(checkpoint['failed_stocks'])} stocks")
    print(f"  📈 Records inserted: {checkpoint['total_records_inserted']:,}")
    print(f"  🔄 Remaining: {len(remaining_stocks)} stocks")
    print(f"  ⏱️  Runtime: {(time.time() - checkpoint['start_time'])/3600:.1f} hours")

    if checkpoint["failed_stocks"]:
        print(f"  ⚠️  Failed stocks: {', '.join(checkpoint['failed_stocks'])}")

    print(f"\n🚀 Continuing with {len(remaining_stocks)} remaining stocks...")

    # Process remaining stocks in batches
    batch_size = 20
    remaining_list = list(remaining_stocks.items())

    for i in range(0, len(remaining_list), batch_size):
        batch = dict(remaining_list[i : i + batch_size])
        batch_num = checkpoint["last_batch"] + 1

        print(f"\n📦 Processing batch {batch_num} ({len(batch)} stocks)")
        print("=" * 60)

        await populate_all_stocks(batch, checkpoint)

        # Update batch number
        checkpoint["last_batch"] = batch_num
        save_checkpoint(checkpoint)

        if i + batch_size < len(remaining_list):
    ...
    print(f"❌ Failed stocks: {len(checkpoint['failed_stocks'])}")
    print(
        f"⏱️  Total runtime: {(time.time() - checkpoint['start_time'])/3600:.1f} hours"
    )

    if checkpoint["failed_stocks"]:
        print(f"\n⚠️  Failed stocks ({len(checkpoint['failed_stocks'])}):")
        for stock in checkpoint["failed_stocks"]:
            print(f"  - {stock}")
