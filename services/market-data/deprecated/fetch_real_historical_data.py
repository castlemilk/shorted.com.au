...
def fetch_stock_data_sync(
    stock_code: str, yf_ticker: str, years: int = 10
) -> List[Dict]:
            ...
            data.append(
                {
                    ...
                    "adjusted_close": round(
                        float(row["Close"]), 2
                    ),  # yfinance data is already adjusted
                    "volume": int(row["Volume"]) if not pd.isna(row["Volume"]) else 0,
                }
            )

        print(
            f"  ✅ Fetched {len(data)} records (from {data[0]['date'] if data else 'N/A'} to {data[-1]['date'] if data else 'N/A'})"
        )
        return data
...
async def update_stock_in_database(
    conn, stock_code: str, yf_ticker: str, years: int = 10
):
    ...
    if existing_data["count"] > 0:
        print(
            f"  📊 Existing data: {existing_data['count']} records from {existing_data['earliest']} to {existing_data['latest']}"
        )

        # Skip if we already have recent data (less than a week old)
        if existing_data["latest"] and existing_data[
            "latest"
    ...
    print(
        f"  ✅ {stock_code}: {inserted} new, {updated} updated, {len(stock_data)} total records processed"
    )
    return inserted
...
async def update_all_stocks():
    ...
    try:
        ...
        for i, (stock_code, yf_ticker) in enumerate(ASX_STOCKS.items(), 1):
            ...
            new_records = await update_stock_in_database(
                conn, stock_code, yf_ticker, 10
            )
        ...
        print(
            f"📅 Overall date range: {date_range['earliest']} to {date_range['latest']}"
        )
        print(
        ...
        for row in final_stats:
            if row["earliest"] and row["latest"]:
                ...
                print(
                    f"{row['stock_code']:<6} | {row['record_count']:>7,} | {row['earliest']} | {row['latest']} | {years:>4.1f}"
                )
            else:
                print(
                    f"{row['stock_code']:<6} | {row['record_count']:>7,} | {'N/A':<10} | {'N/A':<10} | {'N/A':>4}"
                )
