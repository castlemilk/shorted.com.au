            ...
            data.append(
                {
                    ...
                    "adjusted_close": round(
                        float(row["Close"]), 2
                    ),  # yfinance already adjusts
                    "volume": int(row["Volume"]) if not pd.isna(row["Volume"]) else 0,
                }
            )

        print(
            f"  ✅ Fetched {len(data)} records for {stock_code} (from {data[0]['date'] if data else 'N/A'} to {data[-1]['date'] if data else 'N/A'})"
        )
        return data
...
async def update_database_with_historical_data(years: int = 10):
    ...
    try:
        ...
        for stock_code, yf_ticker in ASX_STOCKS.items():
            ...
            for i in range(0, len(stock_data), batch_size):
                ...
                for record in batch:
                    try:
                        ...
                        print(
                            f"    ⚠️  Failed to insert record for {record['date']}: {e}"
                        )
                        continue
            ...
            successful_stocks += 1
            print(
                f"  ✅ Inserted {inserted}/{len(stock_data)} records for {stock_code}"
            )
        ...
        for row in stocks_with_data:
            print(
                f"{row['stock_code']:<6} | {row['record_count']:>7,} | {row['earliest']} | {row['latest']}"
            )
