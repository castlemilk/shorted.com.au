...
//   - zoning: NSW EPI Land Zoning (CC BY), Vicmap Planning plan_zone
...
// suburb has no zoning at all (uncovered, or a state with no source), else the
// measured share with an absent family meaning a genuine 0.
func (row PlanningRow) FamilyShare(family string) *float64 {
	if row.ZoningCoveragePct == nil || *row.ZoningCoveragePct <= 0 {
		return nil
	}
