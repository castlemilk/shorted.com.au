...
func (m *SyncManager) syncStock(ctx context.Context, symbol string) (int, error) {
	...
	if startDate.After(time.Now()) {
		log.Printf("⏭️ %s already up to date", symbol)
		return 0, nil
	}
	...
	for _, p := range m.providers {
		records, syncErr = p.FetchHistoricalData(ctx, symbol, startDate, time.Now())
		...
		// Wait before trying next provider if rate limited
		time.Sleep(p.GetRateLimit())
	}
