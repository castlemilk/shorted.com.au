...
func (s *Store) StartRun(ctx context.Context, runID string, total, priorityCount int) error {
	...
	// Check column types to determine which schema version we're using
	var processedColType string
	var hasPriorityColumns bool
	err = s.db.QueryRow(ctx, `
		SELECT 
			COALESCE((SELECT data_type FROM information_schema.columns 
				WHERE table_name = 'sync_status' AND column_name = 'checkpoint_stocks_processed'), 'none'),
			EXISTS (
				SELECT 1 FROM information_schema.columns 
				WHERE table_name = 'sync_status' 
				AND column_name = 'checkpoint_priority_total'
			)
	`).Scan(&processedColType, &hasPriorityColumns)
	if err != nil {
		// If we can't check, assume old schema (TEXT[]) and no priority columns
		processedColType = "ARRAY"
	...
	if exists {
		...
		if processedColType == "integer" {
			...
			// Old schema: TEXT[] columns - use array_length for counts
			_, err = s.db.Exec(ctx, `
				UPDATE sync_status SET
					status = 'running',
					started_at = CURRENT_TIMESTAMP,
					checkpoint_stocks_total = $2,
					checkpoint_stocks_processed = ARRAY[]::TEXT[],
					checkpoint_stocks_successful = ARRAY[]::TEXT[],
					checkpoint_stocks_failed = ARRAY[]::TEXT[]
				WHERE run_id = $1
			`, runID, total)
		}
	} else {
		...
		if processedColType == "integer" {
			...
			// Old schema: TEXT[] columns
			_, err = s.db.Exec(ctx, `
				INSERT INTO sync_status (
					run_id, status, started_at,
					checkpoint_stocks_total,
					checkpoint_stocks_processed, checkpoint_stocks_successful, checkpoint_stocks_failed
				) VALUES ($1, 'running', CURRENT_TIMESTAMP, $2, ARRAY[]::TEXT[], ARRAY[]::TEXT[], ARRAY[]::TEXT[])
			`, runID, total)
		}
	}
	return err
...
func (s *Store) UpdateProgress(ctx context.Context, runID string, processed, successful, failed, priorityProcessed int) error {
	...
	if processedColType == "integer" {
		...
		// Old schema: TEXT[] columns - we can't update counts directly, skip for now
		// The old schema used arrays to track which stocks were processed
		// For now, just update status based on total
		_, err = s.db.Exec(ctx, `
			UPDATE sync_status SET
				status = CASE
					WHEN $2 >= checkpoint_stocks_total THEN 'completed'
