...
const (
	// settleDelay lets the directory page's animations/JS finish before the
	// download button is clicked.
	settleDelay = 3 * time.Second
	// downloadDeadline bounds the wait for the download to land.
	downloadDeadline = 90 * time.Second
)
...
func (s *ASXScraper) DownloadCSV(ctx context.Context) (string, error) {
	...
	// Create browser context with download path. Named browserCtx, not
	// `context` as in the standalone service — that shadowed the context
	// package and blocked any use of context.* below it.
	...
	//nolint:staticcheck // playwright-go deprecates page.* in favour of locators; migrating the
	// selector flow is a behaviour change this port deliberately does not make.
	...
	// Wait a bit for any animations/JavaScript to settle. A select on ctx (not
	// time.Sleep) so a SIGTERM mid-settle lands promptly instead of after 3s.
	select {
	case <-ctx.Done():
		return "", ctx.Err()
	...
	deadline := time.Now().Add(downloadDeadline)
...
func (s *ASXScraper) DownloadDirect(ctx context.Context, csvURL string) (string, error) {
	...
	// Close is checked, not deferred-and-dropped: a failed flush on the final
	// write is exactly how a truncated CSV reaches GCS silently.
