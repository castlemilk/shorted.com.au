-- Prefer the minified icon variant (logo_icon_gcs_url, 128px) over the full

-- logo in every list-serving materialized view. Falls back to the full logo

-- where no icon exists. Icons backfilled 2026-07-20 for all companies.
...
WITH latest_shorts AS (
 ...
 SELECT ls.stock_code,
    ...
    COALESCE(NULLIF(cm.logo_icon_gcs_url, ''::text), cm.logo_gcs_url, ''::text) AS logo_url,
...
WITH latest_date AS (
 ...
 SELECT rs."PRODUCT_CODE" AS product_code,
    ...
    cm.company_name,
    cm.industry,
    COALESCE(NULLIF(cm.logo_icon_gcs_url, ''::text), cm.logo_gcs_url) AS logo_gcs_url,
