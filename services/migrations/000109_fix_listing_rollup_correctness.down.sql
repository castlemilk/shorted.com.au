-- Restore the 000077/000086 listing rollups: unbounded sold rows, portal-ID
-- fallback for missing address keys, unsuppressed listing prices, and agency
-- agent-name samples.
...
SELECT a.region_code, a.dropped_listing_count, a.avg_drop_pct,
       a.median_drop_pct, a.max_drop_pct, a.max_drop_abs, a.dropped_value,
...
WITH ev AS (
    ...
    SELECT COALESCE(state_code, 'AU') AS state_code,
           ...
           COUNT(*) FILTER (WHERE is_active AND listing_status IN ('for_sale', 'under_offer')) AS for_sale_count,
           COUNT(price) FILTER (WHERE is_active AND listing_status IN ('for_sale', 'under_offer')) AS for_sale_priced,
           AVG(price) FILTER (WHERE is_active AND listing_status IN ('for_sale', 'under_offer')) AS avg_asking,
           PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY price)
               FILTER (WHERE is_active AND listing_status IN ('for_sale', 'under_offer') AND price IS NOT NULL) AS median_asking,
           COUNT(*) FILTER (WHERE listing_status = 'sold' AND price IS NOT NULL) AS sold_count,
           AVG(price) FILTER (WHERE listing_status = 'sold') AS avg_sold,
           PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY price)
               FILTER (WHERE listing_status = 'sold' AND price IS NOT NULL) AS median_sold,
...
SELECT l.state_code,
       ...
       d.avg_drop_pct, d.median_drop_pct, d.max_drop_pct,
       ...
       l.for_sale_count, l.for_sale_priced, l.avg_asking, l.median_asking,
       l.sold_count, l.avg_sold, l.median_sold, l.suburbs_tracked
FROM l
...
WITH base AS (
    ...
    SELECT source, agency_id, MAX(agency_name) AS agency_name, state_code,
           ...
           COUNT(price) FILTER (WHERE is_active AND listing_status IN ('for_sale', 'under_offer')) AS priced_listings,
           AVG(price) FILTER (WHERE is_active AND listing_status IN ('for_sale', 'under_offer')) AS avg_asking,
           PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY price)
               FILTER (WHERE is_active AND listing_status IN ('for_sale', 'under_offer') AND price IS NOT NULL) AS median_asking,
