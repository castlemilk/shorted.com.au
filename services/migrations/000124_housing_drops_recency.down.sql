-- Reverse 000124: restore the 000107 refresh function (no bookkeeping writes),
-- the 000109 listing-rollup definitions (no recency gate, no coverage
-- columns), a NOT NULL drop-index median, and drop housing_mv_refresh.
--
-- Order matters: the function stops writing housing_mv_refresh before the
-- table is dropped. Withheld index medians (NULL) are restored as 0, the
-- pre-000124 "no depth to report" value, because NOT NULL cannot be re-armed
-- over NULLs.
...
-- ---------------------------------------------------------------------------
-- Refresh function, verbatim from 000107.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Listing rollups, verbatim from 000109.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Drop-index median back to NOT NULL (withheld medians restored as 0).
-- ---------------------------------------------------------------------------
UPDATE housing_drop_index_daily
SET median_drop_pct = 0
WHERE median_drop_pct IS NULL;

COMMENT ON COLUMN housing_drop_index_daily.median_drop_pct IS NULL;

ALTER TABLE housing_drop_index_daily ALTER COLUMN median_drop_pct SET NOT NULL;

DROP TABLE IF EXISTS housing_mv_refresh;
