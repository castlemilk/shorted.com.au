...
- `main.go` — `-mode official|crawl|refresh|all` (default `all` = official ingest + MV refresh). `runOfficial()` runs the official-source `jobs` slice; for each: fetch → `upsertRegions` → `upsertObservations` → `updateRun` cursor. A materialized-view refresh failure always exits non-zero, as does exceeding `HOUSING_OFFICIAL_MAX_FAILURES` (Terraform default `15`, so all 16 current sources must fail before the default policy trips; oversized values are bounded so total failure can never report success). 15-minute context timeout. `crawl` mode is opt-in and never in the default schedule.
...
- `GetSuburbProfile(sal_code)` → `{ SuburbSummary, SuburbDemographics, ComparisonBaselines, SuburbCrime? }` — the profile page (full demographics + comparison bars + an absent-when-uncovered crime block). `ComparisonBaselines` carries the ABS established-house medians for the state's capital and rest of state (from `mv_housing_headline`), the median of the state's VG suburb medians, the median suburb's household income (state + national), and `state_census` (dwelling/resident-weighted state shares for the household cards). `national_median_price` is always 0: no open national median house price exists, and the old value averaged NSW+VIC+SA suburbs under an "AU" label.
...
`-mode enqueue` historically posted the **whole** catalog every run. As the catalog grew (25 → 115 → **500** suburbs) that became wasteful on a single residential IP with no proxies, and two gaps showed: each `-mode agent` run only claims up to `CRAWL_AGENT_MAX_JOBS` (default 20), so one drain left the queue partly full; and nothing noticed when the board silently stopped updating (it once sat ~3 days stale unnoticed). The scheduler **right-sizes demand** — no proxy spend — with a daily delta, a periodic full, a drain-until-empty loop, and a freshness alarm.

- **Delta enqueue** (`crawl_delta.go`, `CRAWL_ENQUEUE_SELECTION=delta`). `queryCatalogFreshness` reads each catalog suburb's freshness + churn **read-only** from prod — two cheap aggregates over `property_listings` (max `last_seen_at` + active count) and `property_price_events` (recent event count), joined on a normalised `lower(suburb)|upper(state)|trim(postcode)` key so catalog `Display` vs stored `suburb` casing still matches (a mismatch → treated as never-crawled → still selected, the safe direction). `selectDeltaSuburbs` (pure, unit-tested) selects a suburb if **never crawled**, **stale** (`last_crawled < now − CRAWL_DELTA_TTL_HOURS`, default 24h), or **churny** (`>= CRAWL_DELTA_CHURN_MIN` events, default 1, over `CRAWL_DELTA_CHURN_DAYS`, default 7d); ranks never-first → churniest → oldest; caps at `CRAWL_DELTA_MAX_SUBURBS` (default 120 — 500 suburbs rotate in ~4.2 days) and **logs the capped-off tail** (no silent truncation). `CRAWL_ENQUEUE_SELECTION=all` (the default) preserves the whole-catalog behaviour.
- **Freshness alarm** (`crawl_freshness.go`, `-mode freshness`, READ-ONLY). `classifyFreshness` (pure, unit-tested) reduces the same freshness rows to freshest/median/oldest **covered**-suburb age + the never-crawled coverage gap + a count stale past the TTL. It **alarms** (process **exit 6** + best-effort POST to `CRAWL_FRESHNESS_WEBHOOK`, Slack/Discord-shaped) when the **oldest covered suburb** exceeds `CRAWL_FRESHNESS_ALARM_HOURS` (default 120h — above the ~101h rotation the 120-suburb cap implies, so a healthy rotation cannot alarm). Never-crawled suburbs are a coverage gap, NOT an alarm (else a partially-seeded catalog alarms forever); a fresh/never-run env with zero coverage never alarms. This is the guard that catches the board silently going stale.
- **Drain-until-empty** (shell, `deploy/housing-crawl-common.sh` `hc_drain_until_empty`). Loops `-mode agent` until the queue reports empty — parsing the collector's stable `[agent] no more jobs` / `[agent] done: processed N job(s)` lines (pinned by a comment in `runAgent`) — bounded by `CRAWL_DRAIN_MAX_ROUNDS` (default 30) and honoring the exit-3 re-warm / exit-4 Chrome breaks. Chosen over a Go re-claim loop so each `-mode agent` round keeps its per-run semantics (self-warm Chrome + end-of-run MV refresh + revalidate ping); the safety cap stays a cap.
- **Wrappers + launchd** (`deploy/`). `run-housing-delta.sh` (daily 03:00, `com.shorted.housing-delta.plist.template`) = delta enqueue → drain → freshness. `run-housing-full.sh` (fortnightly 1st/15th 02:00, `com.shorted.housing-full.plist.template`) = full enqueue → drain → freshness. Both source `~/.shorted-housing-crawl.env` and set conservative pacing (`CRAWL_MIN_DELAY_MS`/`CRAWL_MAX_DELAY_MS` 20–45s). Install/env details + the full knob table: `deploy/README.md` ("Demand-right-sizing scheduler"). The older `run-housing-agent.sh` (single drain, whole-catalog enqueue) still works unchanged.

### 6.10 Crawl health → the admin jobs dashboard (`crawl_run_status`)

The admin **jobs dashboard** (`/admin`, backed by `GET /api/admin/jobs` → `jobmonitor.Collect()`) reports the whole scheduled fleet by reading **GCP Cloud Run Job executions + Cloud Scheduler triggers directly** — it is structurally **blind to anything that does not run on GCP**. The residential crawl runs on a **Mac** (Kasada needs a residential IP + a warm host-Chrome, so it **cannot** run on Cloud Run), so it would never appear there without a DB-backed source. Migration **`000089_crawl_run_status`** adds that source, and the crawl now shows up as a first-class scheduled job (type `rig`, category "Housing crawl") next to the Cloud Run jobs.

- **The record** (`crawl_run_status`, PK `(run_type, host)`): one health row per scheduled crawl **run-type** (`delta` / `full` / `agent`) per **rig**. Written best-effort by the collector — `crawl_run_status.go` (`upsertCrawlRunStatus` / `updateCrawlRunFreshness` / `deriveCrawlRunStatus`). A launchd wrapper exports `CRAWL_RUN_TYPE` (delta|full) and a **stable `CRAWL_RUN_ID`**; `runAgent` writes one row per round at the end (status derived from that round's counts: `rewarm` → `rewarm`, blocked-with-zero-events → `blocked`, some-blocked-some-events → `partial`, a claim failure that did nothing → `error`, else `ok`), and rounds sharing the run-id **accumulate** their counts (`suburbs`, `listings`, `events`, `blocked`) into the one row while refreshing `finished_at=now()`. `-mode freshness` annotates the same row with `freshness_oldest_hours`. Writes never fail the crawl.
- **The merge** (`services/shorts/.../crawl_jobs.go`): the `/api/admin/jobs` handler appends `s.crawlJobStatuses()` — the `crawl_run_status` rows mapped into the same `jobmonitor.JobStatus` shape as the Cloud Run jobs — after the GCP list. `GetCrawlRunStatuses` (store) degrades gracefully (empty) when the table is absent (pre-migration / dev DB).
- **Dead-rig detection** (`crawlRunHealth`, pure + unit-tested): a live rig re-writes `finished_at` every round; a rig that **stops** (crashed launchd, machine off, Chrome permanently wedged) stops writing, so `finished_at` goes stale. Health escalates a last-`ok` row **warning** once it passes the run-type's cadence (delta > 30h, full > 16d) and **critical** past 2× that horizon — no heartbeat job needed, the **absence of writes IS the signal**. `error`/`blocked` → critical; `rewarm`/`partial` → warning (critical if also dead). Scheduling stays on **launchd** (Kasada); only the **tracking** now matches the GCP jobs.
