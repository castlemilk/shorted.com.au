...
- **`refresh_housing_materialized_views()` lacks the 000095 guard pattern**
  (above).
- ~~**4 committed testdata files carry real portal page content**~~ Resolved:
  `services/house-price-collector/testdata/{rea,domain}-pagemeta.html` are now
  synthetic, generated by `scripts/generate-synthetic-housing-pagemeta-fixtures.mjs`
  and guarded by `scripts/check-portal-content-provenance.mjs`. The other two
  copies went with the retired `services/jobs` houseprices port (2026-09-24).
