...
- **The edge anonymous bucket may throttle real agent sessions.** On `api.shorted.com.au` everything except `/health`, `/healthz` and `/api/auth*` is rate-limit eligible (`worker.js:716-720`), so an anonymous MCP client is keyed `a:<ip>` at **10 requests/10s burst, 30/60s sustained**. A handshake is only 2-4 POSTs, but an agent working through several tool calls can plausibly reach 10 in 10 seconds — and with no app-layer limits until Phase 3, that edge bucket is also the *only* protection this endpoint has. **Task 10 must measure a real client session against it** and record the decision either way: raise the ceiling for `/mcp`, give it its own bucket, or accept the throttle. Do not silently exempt `/mcp` — that would leave an unauthenticated tool surface with no ceiling at all.
