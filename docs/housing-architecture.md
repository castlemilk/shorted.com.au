...
### G. **Broaden suburb price coverage** (BACKLOG — drives the `/housing/[state]` choropleth)
**Problem:** the suburb choropleth colours by `median_price` (`dwelling_type='house'`), but that exists for only ~332 SA suburbs today (Valuer-General SA, quarterly) out of ~15k AU suburbs. So every state map renders mostly as the light-grey "no data" hatch with a small coloured cluster (e.g. SA = Adelaide metro). The map + RPCs + join are correct; this is purely **data sparsity**. Populate price as broadly as licensing allows, in rough priority:
1. **VIC (already ingested, just not surfaced):** VIC Valuer-General data is **annual**, so it's stored in `house_prices` but excluded from `mv_housing_headline` (which filters `period_freq='Q'`). Either include annual rows in the MV (add an `'A'`-aware path) **or** change the suburb price join in `postgres_house_prices.go` (`ListStateSuburbs`/`GetSuburbProfile`) to read the latest `house_prices.median_price` regardless of `period_freq`, falling back from Q→A. Lights up VIC immediately.
2. **Other states' Valuer-General / open data:** NSW Valuer-General (open VG bulk; per [[housing-residential-crawl]] NSW is the most viable open source), QLD, WA, TAS. Add per-state ingest in the collector (`source='<st>_vg'`), keyed to SAL via the existing `region_code↔sal_code` bridge (migration 000056 pattern).
3. **Crawl tier (REA/Domain) — last resort, licence-gated:** §E above; `source_licence='proprietary-tos-restricted'`, never republished, only fills gaps the open sources can't.
**Acceptance:** a state map (e.g. NSW) shows the majority of metro suburbs coloured, not a single cluster. Until then the grey "no data" hatch is the correct, honest rendering.
