...
`services/house-price-collector/crawl*.go` runs a **live, supplementary** crawl of realestate.com.au (Kasada) and domain.com.au (Akamai): suburb-aggregate medians (`-mode crawl`) and individual for-sale listings (`-mode listings`, which diffs asking prices across runs into price-drop events in `property_listings`/`property_price_events`). It is fail-safe by design — it never blocks the ABS/RBA backbone and never stores an unvalidated value — and its brandbrain boundary is counts/aggregates-only: listings jobs submit tallies, while the medians tier projects rendered pages down to an allowlist of aggregate metrics and pagination counts before calling ExtractRealEstate. Raw portal HTML, listing IDs, addresses, prices and agents stay on the rig. Unlike earlier iterations of this doc, it **does now actually scrape**: verified live, e.g. a South Yarra sweep returned 132 REA + 110 Domain listings with `blocked=0`.
