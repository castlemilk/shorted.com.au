...
## Housing (feature + price tracker + suburb explorer + price drops)
...
3. **The suburb explorer** (`/housing` national states map → `/housing/[state]` suburb choropleth + list → `/housing/[state]/[suburb]` profile) — a national→state→suburb **choropleth drilldown** over real ABS TopoJSON boundaries, with a **"Colour by" metric toggle** across house price, ABS Census demographics + culture (religion/language/born-overseas), **electoral representation** (federal + state member/party + two-party-preferred lean), and gated pooled-latest **crime ranks** (break-ins/violent/motor vehicle; yellow→red danger ramp; rank 0 → no-data hatch). `suburb_demographics` + gated `mv_suburb_crime_latest` (keyed by ABS `sal_code`) → `ListStateSuburbs`/`GetSuburbProfile` RPCs. **LIVE on prod.**
...
That crawl corpus now powers **`/price-drops`** (PR #315) — a flagship price-cuts board over four rollups (**state / suburb / address / agency**) built by migration `000086_price_drops_rollups`: `mv_state_price_drops` + `mv_agency_stats` + a **rebuilt** `mv_suburb_price_drops` with a **≤40% drop-pct sanity cap** (listing typos) and **cross-portal address dedup** (a dual-listed cut counts once). RPCs `GetPriceDropsOverview` + `ListAgencyPriceStats`; the **agency** RPC is flag-gated behind `HOUSING_DROP_LISTINGS_ENABLED` (kill switch, **ON by default**) because it carries agency/agent names, and `mv_agency_stats` suppresses drop depth until an agency has **≥3 dropped addresses** (k-anon). `/housing/drops` **308-redirects** to `/price-drops`.
...
| `web/src/app/price-drops/page.tsx` + `housing/drops/page.tsx` | Price-cuts board — **static ISR (1h)** + `cache:housing:drops:*` KV (24h); `?state=` deep link read **client-side** (`useSearchParams` under Suspense — a server-page `searchParams` read would force dynamic). `/housing/drops` = `permanentRedirect` (308) → `/price-drops` |
...
| `services/migrations/000053…000092` (housing) | `000053/054` price tracker (`house_prices` EAV + `mv_housing_headline` + licence gate); `000055` `suburb_demographics` + `house_price_regions.sal_code`; `000056` sal_code backfill; `000057` culture; `000058/059/060` federal/state-district/state-member electoral; `000084` suburb-banner cols on `suburb_demographics` (**renumbered off a stale 000083** after colliding with `000083_add_state_exposure`); `000086` price-drops rollups (`mv_state_price_drops` + `mv_agency_stats` + rebuilt `mv_suburb_price_drops`); `000090` `suburb_crime_stats` + initial `mv_suburb_crime_latest`; `000092` deterministic gated crime MV rebuild (`NOT small_pop AND NOT unreliable`, excludes non-commercial WA) + guarded refresh wiring |
| `services/house-price-collector/` | `main.go` (`-mode official\|vg-nsw\|census\|electorates\|crawl\|listings\|agent\|enqueue\|banners\|warmcheck\|refresh\|all`), `abs.go`, `rba.go`, `census*.go`, `electorates.go`, `banners.go` (`-mode banners`: archetype → `suburb_demographics.banner_*` from committed `web/public/geo/insights/suburb-archetypes.json`, AVIF bg lib `web/public/housing-banners/bg/`), `official_freshness.go` (persisted `MAX(period)` VG missing/stale alarm + exit 1), `revalidate.go` (`pingRevalidate` → `/api/revalidate?path=/price-drops,/housing&flush=housing` after a run's MV refresh; no-op when `REVALIDATION_URL`/`_SECRET` unset), `store.go` |
...
| `services/house-price-collector/deploy/run-housing-vg-nsw.sh` | Residential-only NSW Valuer-General PSI runner (`-mode vg-nsw`) + monthly launchd template; env-driven, no browser/challenge bypass, exit 1 on ingest/freshness failure |
...
- **LIVE** (collector fetches each run): ABS `RES_DWELL_ST` (mean_price, total_value — national+states), ABS `RES_DWELL` (median_price by dwelling for GCCSAs), ABS `RPPI` (price_index, but frozen at 2021-Q4 upstream), RBA `E2` (debt_to_income), and state Valuer-General suburb medians (`vg_sa`, dynamically discovered `vg_vic`; residential-only `-mode vg-nsw`). VG missing/stale persisted periods are fatal to the scheduled official run. All are attribution-licensed as recorded per row.
...
- **Crawl-aligned caching** (PR #318): `/price-drops` is **static ISR (1h)** + Upstash `cache:housing:drops:*` (24h); the collector's `pingRevalidate` busts both the instant a run's MV refresh lands (`?path=/price-drops,/housing&flush=housing` — `/api/revalidate`'s `flush=` now accepts **comma-separated families** `shorts,housing`). Read the `?state=` deep link **client-side** (`useSearchParams` under Suspense) — reading `searchParams` in the server page silently forces dynamic and kills the ISR. Rigs read `REVALIDATION_URL`/`_SECRET` from `~/.shorted-housing-crawl.env`; the Cloud Run job from TF `manage_revalidation_secret` — both **no-op when unset**.
- **Suburb banners** (PR #308): `banner_*` fields hydrate on **`GetSuburbProfile` only** (not the list RPCs); `-mode banners` classifies purely from the committed `suburb-archetypes.json` (no crawl), AVIF backgrounds under `web/public/housing-banners/bg/` (migration `000084`).
