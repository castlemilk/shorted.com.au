...
| `services/house-price-collector/` | `main.go` (`-mode official\|census\|electorates\|crawl\|listings\|agent\|enqueue\|banners\|warmcheck\|refresh\|all`), `abs.go`, `rba.go`, `census*.go`, `electorates.go`, `banners.go` (`-mode banners`: archetype → `suburb_demographics.banner_*` from committed `web/public/geo/insights/suburb-archetypes.json`, AVIF bg lib `web/public/housing-banners/bg/`), `revalidate.go` (`pingRevalidate` → `/api/revalidate?path=/price-drops,/housing&flush=housing` after a run's MV refresh; no-op when `REVALIDATION_URL`/`_SECRET` unset), `store.go` |
...
- **CRAWL** (`source_licence='proprietary-tos-restricted'`, never republished): `crawl_rea`, `crawl_domain` — **LIVE**; agents write `property_listings` + `property_price_events` to shorted prod (counts-only summaries back to brandbrain), while the medians tier sends only an allowlisted aggregate/count projection to ExtractRealEstate. Raw portal HTML and `CRAWL_TRACE` screenshot/HTML artifacts stay **local-only, gitignored, never uploaded**.
