...
A **Tier-3 residential listing crawl** of REA/Domain is **LIVE**: a `crawl_jobs` queue in brandbrain (`api.brandbrain.dev`, no listing PII crosses over — counts-only) is populated by `-mode enqueue` and drained by `-mode agent` pollers on residential Macs, which crawl via Playwright `ConnectOverCDP` to a dedicated, REA-URL-warmed host Chrome — native Chrome startup navigation (not Playwright) is what actually clears REA's Kasada challenge. Reliability is self-healing: `-mode warmcheck` proves the session is warm and `deploy/run-housing-crawl.sh` auto-launches/re-warms Chrome before each run (see `docs/housing-architecture.md` §6).
