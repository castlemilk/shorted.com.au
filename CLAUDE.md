...
| `services/migrations/000053…000086` (housing) | `000053/054` price tracker (`house_prices` EAV + `mv_housing_headline` + licence gate); `000055` `suburb_demographics` + `house_price_regions.sal_code`; `000056` sal_code backfill; `000057` culture; `000058/059/060` federal/state-district/state-member electoral; `000084` suburb-banner cols on `suburb_demographics` (**renumbered off a stale 000083** after colliding with `000083_add_state_exposure`); `000086` price-drops rollups (`mv_state_price_drops` + `mv_agency_stats` + rebuilt `mv_suburb_price_drops`) |
