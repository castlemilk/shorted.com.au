...
- **Editor** (`editor.ts`): builds the signal board (pool 30), applies the pure novelty gate `hasNewDevelopment` — covers a stock only if never covered OR a price-sensitive headline / director trade post-dates the last take OR |Δ90d short| ≥ 2.0pp. **Dedup guard** (`journalism.ts` `hasRecentUnpublishedDraft`, `DRAFT_SUPPRESS_DAYS`=7, env `NEWSROOM_DRAFT_SUPPRESS_DAYS`): the editor also skips any stock that already has an *unpublished* draft < 7 days old — without it the |Δ90d short| ≥ 2pp branch re-fires daily for volatile names (LOT, ELD…), and because each day's headline yields a new slug the `ON CONFLICT (slug)` upsert never matches, so a fresh near-duplicate draft is INSERTed every morning. A volatile stock is NOT re-commissioned until its waiting draft is published or deleted. Gemini (`EDITOR_MODEL`, default `gemini-3.5-flash`) commissions up to `MAX_TAKES_PER_DAY` (10) takes + `MAX_DEEPDIVES_PER_DAY` (2) deep-dives, caps enforced defensively in code.
...
| `publish-content --slug=SLUG [--no-images] [--no-validate]` | Import ONE `content/news` article by frontmatter slug, then run the `publish` chain. The entrypoint of the prod `shorted-news-publish` Cloud Run job, which `POST /api/admin/news/publish` (or `CONFIRM=prod task news:publish:remote SLUG=…`) runs with no local credentials — see `SERVERLESS.md`. | ~$0.32 with images |
...
2. **Review** in the terminal:
   ...
   - **Manual runs collide with the morning cron.** Running `newsroom-daily` by hand on a weekday the cron already ran produces a *second* same-day draft set for the same volatile stocks. Check `list-drafts` first; if today's cron set already exists, review/publish it rather than generating another.
   - **Dedup before publishing.** The dedup guard stops *new* pileup, but a backlog can still hold older stale drafts per stock. For a stock with multiple drafts, keep the freshest (or best-cited) and delete the rest first — otherwise the queue carries near-duplicates and the guard keeps that stock suppressed. No built-in prune command yet; delete by slug via psql: `DELETE FROM editorial_takes WHERE published_at IS NULL AND slug IN (...)` (preview with a `SELECT` first; never touch a row whose `published_at` is set).
...
1. **Preview**: `newsroom-preview --stock=CODE`. Judge:
   ...
   - Voice: no hedging filler ("it remains to be seen"), figures not spelled out ($600M not "six-hundred million"), headline leads with the short-story observation (never a bare generic "Shorts …"), sentence case, no AI-tell verbs (Reacts to / Faces).
   - **Precision (these recur — check every time):** a short-interest *move* is in **percentage points** ("+2.8pp" / "2.8 percentage points"), never "+2.8%"; money takes a leading symbol (**$5.27 / A$83M**, never "5.27 AUD"); a short bet is a **"position"**, never a "stake"; `<TimelineEvent>` `date` matches the cited ref's stored date; no literal `\n`/`\t` printed in the body. The writer prompt now bakes in the pp/currency/stake rules, but the model still slips — verify.
   - Grounding line: 0 dropped, ≥1 citation, the "would auto-publish" decision. Note an uncited short %/slope/correlation/peer-average/price-move is CORRECT (Shorted-own overview number) — don't flag it; only news facts and reported financials need `[ref-N]`.
