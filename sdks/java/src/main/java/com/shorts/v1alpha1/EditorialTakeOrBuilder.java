// Generated by the protocol buffer compiler.  DO NOT EDIT!
// NO CHECKED-IN PROTOBUF GENCODE
// source: shorts/v1alpha1/news.proto
// Protobuf Java Version: 4.35.1

package com.shorts.v1alpha1;

@com.google.protobuf.Generated
public interface EditorialTakeOrBuilder extends
    // @@protoc_insertion_point(interface_extends:shorts.v1alpha1.EditorialTake)
    com.google.protobuf.MessageOrBuilder {

  /**
   * <code>string id = 1 [json_name = "id"];</code>
   * @return The id.
   */
  java.lang.String getId();
  /**
   * <code>string id = 1 [json_name = "id"];</code>
   * @return The bytes for id.
   */
  com.google.protobuf.ByteString
      getIdBytes();

  /**
   * <code>string slug = 2 [json_name = "slug"];</code>
   * @return The slug.
   */
  java.lang.String getSlug();
  /**
   * <code>string slug = 2 [json_name = "slug"];</code>
   * @return The bytes for slug.
   */
  com.google.protobuf.ByteString
      getSlugBytes();

  /**
   * <code>string headline = 3 [json_name = "headline"];</code>
   * @return The headline.
   */
  java.lang.String getHeadline();
  /**
   * <code>string headline = 3 [json_name = "headline"];</code>
   * @return The bytes for headline.
   */
  com.google.protobuf.ByteString
      getHeadlineBytes();

  /**
   * <code>string stock_code = 4 [json_name = "stockCode"];</code>
   * @return The stockCode.
   */
  java.lang.String getStockCode();
  /**
   * <code>string stock_code = 4 [json_name = "stockCode"];</code>
   * @return The bytes for stockCode.
   */
  com.google.protobuf.ByteString
      getStockCodeBytes();

  /**
   * <pre>
   * Markdown body
   * </pre>
   *
   * <code>string body_md = 5 [json_name = "bodyMd"];</code>
   * @return The bodyMd.
   */
  java.lang.String getBodyMd();
  /**
   * <pre>
   * Markdown body
   * </pre>
   *
   * <code>string body_md = 5 [json_name = "bodyMd"];</code>
   * @return The bytes for bodyMd.
   */
  com.google.protobuf.ByteString
      getBodyMdBytes();

  /**
   * <code>string sentiment = 6 [json_name = "sentiment"];</code>
   * @return The sentiment.
   */
  java.lang.String getSentiment();
  /**
   * <code>string sentiment = 6 [json_name = "sentiment"];</code>
   * @return The bytes for sentiment.
   */
  com.google.protobuf.ByteString
      getSentimentBytes();

  /**
   * <pre>
   * UUID of source news_article, or empty
   * </pre>
   *
   * <code>string source_article_id = 7 [json_name = "sourceArticleId"];</code>
   * @return The sourceArticleId.
   */
  java.lang.String getSourceArticleId();
  /**
   * <pre>
   * UUID of source news_article, or empty
   * </pre>
   *
   * <code>string source_article_id = 7 [json_name = "sourceArticleId"];</code>
   * @return The bytes for sourceArticleId.
   */
  com.google.protobuf.ByteString
      getSourceArticleIdBytes();

  /**
   * <pre>
   * External publisher URL (attribution)
   * </pre>
   *
   * <code>string source_url = 8 [json_name = "sourceUrl"];</code>
   * @return The sourceUrl.
   */
  java.lang.String getSourceUrl();
  /**
   * <pre>
   * External publisher URL (attribution)
   * </pre>
   *
   * <code>string source_url = 8 [json_name = "sourceUrl"];</code>
   * @return The bytes for sourceUrl.
   */
  com.google.protobuf.ByteString
      getSourceUrlBytes();

  /**
   * <pre>
   * 'Stockhead', 'Motley Fool', etc.
   * </pre>
   *
   * <code>string source_name = 9 [json_name = "sourceName"];</code>
   * @return The sourceName.
   */
  java.lang.String getSourceName();
  /**
   * <pre>
   * 'Stockhead', 'Motley Fool', etc.
   * </pre>
   *
   * <code>string source_name = 9 [json_name = "sourceName"];</code>
   * @return The bytes for sourceName.
   */
  com.google.protobuf.ByteString
      getSourceNameBytes();

  /**
   * <code>string og_image_url = 10 [json_name = "ogImageUrl"];</code>
   * @return The ogImageUrl.
   */
  java.lang.String getOgImageUrl();
  /**
   * <code>string og_image_url = 10 [json_name = "ogImageUrl"];</code>
   * @return The bytes for ogImageUrl.
   */
  com.google.protobuf.ByteString
      getOgImageUrlBytes();

  /**
   * <code>int32 word_count = 11 [json_name = "wordCount"];</code>
   * @return The wordCount.
   */
  int getWordCount();

  /**
   * <pre>
   * 'gemini-2.0-flash', etc.
   * </pre>
   *
   * <code>string model = 12 [json_name = "model"];</code>
   * @return The model.
   */
  java.lang.String getModel();
  /**
   * <pre>
   * 'gemini-2.0-flash', etc.
   * </pre>
   *
   * <code>string model = 12 [json_name = "model"];</code>
   * @return The bytes for model.
   */
  com.google.protobuf.ByteString
      getModelBytes();

  /**
   * <code>.google.protobuf.Timestamp published_at = 13 [json_name = "publishedAt"];</code>
   * @return Whether the publishedAt field is set.
   */
  boolean hasPublishedAt();
  /**
   * <code>.google.protobuf.Timestamp published_at = 13 [json_name = "publishedAt"];</code>
   * @return The publishedAt.
   */
  com.google.protobuf.Timestamp getPublishedAt();
  /**
   * <code>.google.protobuf.Timestamp published_at = 13 [json_name = "publishedAt"];</code>
   */
  com.google.protobuf.TimestampOrBuilder getPublishedAtOrBuilder();

  /**
   * <code>.google.protobuf.Timestamp created_at = 14 [json_name = "createdAt"];</code>
   * @return Whether the createdAt field is set.
   */
  boolean hasCreatedAt();
  /**
   * <code>.google.protobuf.Timestamp created_at = 14 [json_name = "createdAt"];</code>
   * @return The createdAt.
   */
  com.google.protobuf.Timestamp getCreatedAt();
  /**
   * <code>.google.protobuf.Timestamp created_at = 14 [json_name = "createdAt"];</code>
   */
  com.google.protobuf.TimestampOrBuilder getCreatedAtOrBuilder();

  /**
   * <code>string hero_image_url = 15 [json_name = "heroImageUrl"];</code>
   * @return The heroImageUrl.
   */
  java.lang.String getHeroImageUrl();
  /**
   * <code>string hero_image_url = 15 [json_name = "heroImageUrl"];</code>
   * @return The bytes for heroImageUrl.
   */
  com.google.protobuf.ByteString
      getHeroImageUrlBytes();

  /**
   * <code>repeated .shorts.v1alpha1.InlineImage inline_images = 16 [json_name = "inlineImages"];</code>
   */
  java.util.List<com.shorts.v1alpha1.InlineImage> 
      getInlineImagesList();
  /**
   * <code>repeated .shorts.v1alpha1.InlineImage inline_images = 16 [json_name = "inlineImages"];</code>
   */
  com.shorts.v1alpha1.InlineImage getInlineImages(int index);
  /**
   * <code>repeated .shorts.v1alpha1.InlineImage inline_images = 16 [json_name = "inlineImages"];</code>
   */
  int getInlineImagesCount();
  /**
   * <code>repeated .shorts.v1alpha1.InlineImage inline_images = 16 [json_name = "inlineImages"];</code>
   */
  java.util.List<? extends com.shorts.v1alpha1.InlineImageOrBuilder> 
      getInlineImagesOrBuilderList();
  /**
   * <code>repeated .shorts.v1alpha1.InlineImage inline_images = 16 [json_name = "inlineImages"];</code>
   */
  com.shorts.v1alpha1.InlineImageOrBuilder getInlineImagesOrBuilder(
      int index);

  /**
   * <code>.google.protobuf.Timestamp tweet_published_at = 17 [json_name = "tweetPublishedAt"];</code>
   * @return Whether the tweetPublishedAt field is set.
   */
  boolean hasTweetPublishedAt();
  /**
   * <code>.google.protobuf.Timestamp tweet_published_at = 17 [json_name = "tweetPublishedAt"];</code>
   * @return The tweetPublishedAt.
   */
  com.google.protobuf.Timestamp getTweetPublishedAt();
  /**
   * <code>.google.protobuf.Timestamp tweet_published_at = 17 [json_name = "tweetPublishedAt"];</code>
   */
  com.google.protobuf.TimestampOrBuilder getTweetPublishedAtOrBuilder();

  /**
   * <code>repeated .shorts.v1alpha1.TakeCitation citations = 18 [json_name = "citations"];</code>
   */
  java.util.List<com.shorts.v1alpha1.TakeCitation> 
      getCitationsList();
  /**
   * <code>repeated .shorts.v1alpha1.TakeCitation citations = 18 [json_name = "citations"];</code>
   */
  com.shorts.v1alpha1.TakeCitation getCitations(int index);
  /**
   * <code>repeated .shorts.v1alpha1.TakeCitation citations = 18 [json_name = "citations"];</code>
   */
  int getCitationsCount();
  /**
   * <code>repeated .shorts.v1alpha1.TakeCitation citations = 18 [json_name = "citations"];</code>
   */
  java.util.List<? extends com.shorts.v1alpha1.TakeCitationOrBuilder> 
      getCitationsOrBuilderList();
  /**
   * <code>repeated .shorts.v1alpha1.TakeCitation citations = 18 [json_name = "citations"];</code>
   */
  com.shorts.v1alpha1.TakeCitationOrBuilder getCitationsOrBuilder(
      int index);

  /**
   * <code>repeated .shorts.v1alpha1.LayoutImage layout_images = 19 [json_name = "layoutImages"];</code>
   */
  java.util.List<com.shorts.v1alpha1.LayoutImage> 
      getLayoutImagesList();
  /**
   * <code>repeated .shorts.v1alpha1.LayoutImage layout_images = 19 [json_name = "layoutImages"];</code>
   */
  com.shorts.v1alpha1.LayoutImage getLayoutImages(int index);
  /**
   * <code>repeated .shorts.v1alpha1.LayoutImage layout_images = 19 [json_name = "layoutImages"];</code>
   */
  int getLayoutImagesCount();
  /**
   * <code>repeated .shorts.v1alpha1.LayoutImage layout_images = 19 [json_name = "layoutImages"];</code>
   */
  java.util.List<? extends com.shorts.v1alpha1.LayoutImageOrBuilder> 
      getLayoutImagesOrBuilderList();
  /**
   * <code>repeated .shorts.v1alpha1.LayoutImage layout_images = 19 [json_name = "layoutImages"];</code>
   */
  com.shorts.v1alpha1.LayoutImageOrBuilder getLayoutImagesOrBuilder(
      int index);

  /**
   * <pre>
   * 'markdown' | 'mdx'
   * </pre>
   *
   * <code>string body_format = 20 [json_name = "bodyFormat"];</code>
   * @return The bodyFormat.
   */
  java.lang.String getBodyFormat();
  /**
   * <pre>
   * 'markdown' | 'mdx'
   * </pre>
   *
   * <code>string body_format = 20 [json_name = "bodyFormat"];</code>
   * @return The bytes for bodyFormat.
   */
  com.google.protobuf.ByteString
      getBodyFormatBytes();

  /**
   * <pre>
   * one-sentence dek under the headline
   * </pre>
   *
   * <code>string standfirst = 21 [json_name = "standfirst"];</code>
   * @return The standfirst.
   */
  java.lang.String getStandfirst();
  /**
   * <pre>
   * one-sentence dek under the headline
   * </pre>
   *
   * <code>string standfirst = 21 [json_name = "standfirst"];</code>
   * @return The bytes for standfirst.
   */
  com.google.protobuf.ByteString
      getStandfirstBytes();

  /**
   * <pre>
   * e.g. "The Shorted Desk — Mining &amp; Resources"
   * </pre>
   *
   * <code>string byline = 22 [json_name = "byline"];</code>
   * @return The byline.
   */
  java.lang.String getByline();
  /**
   * <pre>
   * e.g. "The Shorted Desk — Mining &amp; Resources"
   * </pre>
   *
   * <code>string byline = 22 [json_name = "byline"];</code>
   * @return The bytes for byline.
   */
  com.google.protobuf.ByteString
      getBylineBytes();

  /**
   * <code>string hero_caption = 23 [json_name = "heroCaption"];</code>
   * @return The heroCaption.
   */
  java.lang.String getHeroCaption();
  /**
   * <code>string hero_caption = 23 [json_name = "heroCaption"];</code>
   * @return The bytes for heroCaption.
   */
  com.google.protobuf.ByteString
      getHeroCaptionBytes();

  /**
   * <pre>
   * e.g. "AI-generated illustration"
   * </pre>
   *
   * <code>string hero_credit = 24 [json_name = "heroCredit"];</code>
   * @return The heroCredit.
   */
  java.lang.String getHeroCredit();
  /**
   * <pre>
   * e.g. "AI-generated illustration"
   * </pre>
   *
   * <code>string hero_credit = 24 [json_name = "heroCredit"];</code>
   * @return The bytes for heroCredit.
   */
  com.google.protobuf.ByteString
      getHeroCreditBytes();
}
