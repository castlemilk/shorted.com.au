...
- **ShortInterest.au** (the modern playbook): FAQPage + Dataset +
  ...
  gross-short-sales blend** ("see short flow 3 days sooner" — *rejected for us
  on licensing grounds, §7*), and /methodology + /editorial-policy trust pages.
...
| 9 | **Watch (2–6 weeks)**: rankings for "most shorted asx stocks" / "asx short selling"; CTR on the new title; whether the freshness stamp earns a SERP date snippet | — | — | Title retargets take 2–6 weeks to settle. **Do not re-tweak titles before then** |
| 10 | Mobile CWV (Lighthouse LCP), YouTube explainers, short-move alerts | later | L | Strategy Phases 3–4 remainder |
...
## 7. Rejected: ASX T+1 gross short sales (closed 27 July)

**Dropped — do not re-propose.** Spiked before building; the source is real
(`asx.com.au/data/shortsell.txt`, fixed-width, ~11am AEST T+1, ASX + Cboe
consolidated) but three findings closed it:

1. **ASX Terms of Use forbid it.** They bar "modify, copy, reproduce,
   republish … or distribute any content", bar "any spider, screen scraper,
   robot", and bar commercial use without written consent — defining
   commercial as "any use other than … your own personal and private decision
   making". ASX runs a sub-vendor/MarketSource licensing regime and treats
   processed output as a "New Original Work". (robots.txt only disallows
   `/search*`; the ToS, not robots.txt, is the binding constraint.) That
   ShortInterest.au publishes this data is not a licence.
2. **It is gross short-sales FLOW, not short interest** — one day's sell volume
   over issued capital. Not comparable to the ASIC T+4 *position* percentage,
   so it could never be "blended" into our numbers, only shown alongside them.
3. The endpoint is **Imperva/Incapsula-walled** (curl, `stealthhttp` native TLS
   and headless Chromium all blocked).

The strategy doc itself only ever claimed this was *parity* with
ShortInterest.au, not differentiation. The effort belongs on
authority/backlinks (§3 item 2), which is the actual gap.
