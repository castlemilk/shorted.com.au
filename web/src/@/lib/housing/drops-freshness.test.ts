...
describe("timestampToDate", () => {
  it("reads protobuf-es bigint seconds", () => {
    expect(timestampToDate(ts("2026-09-15T01:46:00Z"))?.toISOString()).toBe("2026-09-15T01:46:00.000Z");
  });
...
describe("dropsFreshness", () => {
  it("labels the data date in Sydney time and flags a frozen crawl", () => {
    const f = dropsFreshness({ asOf: ts("2026-09-15T02:00:00Z"), dataThrough: ts("2026-09-15T01:46:00Z") }, NOW);
  ...
  it("is not stale inside the window", () => {
    const recent = new Date(NOW.getTime() - (DROPS_STALE_AFTER_HOURS - 1) * 3_600_000).toISOString();
    expect(dropsFreshness({ asOf: ts(recent), dataThrough: ts(recent) }, NOW).stale).toBe(false);
  });

  it("flags stale when EITHER stamp is old — a refresh over frozen crawl data is still stale", () => {
    const f = dropsFreshness({ asOf: ts("2026-09-23T20:00:00Z"), dataThrough: ts("2026-09-15T01:46:00Z") }, NOW);
    expect(f.stale).toBe(true);
  });
...
describe("stateCoverage", () => {
  it("ranks a state at or above the index's coverage threshold", () => {
    ...
    expect(stateCoverage({ suburbsSwept14d: 101, catalogSuburbs: 135 }).ranked).toBe(true);
    expect(stateCoverage({ suburbsSwept14d: 81, catalogSuburbs: 135 }).ranked).toBe(true);
  });
  ...
  it("keeps ranking when coverage is unknown rather than greying out every state", () => {
    expect(stateCoverage({ suburbsSwept14d: 0, catalogSuburbs: 0 })).toEqual({ ranked: true });
  });
});
