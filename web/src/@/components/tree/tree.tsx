...
const Tree: React.FC<TreeProps> = ({ data }) => {
  ...
  useEffect(() => {
    ...
    const isNodeComplete = (node: d3.HierarchyNode<TreeItem>): boolean => {
      return node.data.status === 'DONE' || (node.children ? node.children.some(isNodeComplete) : false);
    };
    ...
    const renderLinks = (linkData: d3.HierarchyLink<TreeItem>[], className: string, colorFn: (d: d3.HierarchyLink<TreeItem>) => string) => {
      svg
        .selectAll(`.${className}`)
        .data(linkData)
        .enter()
        ...
        .attr('d', d => {
          ...
          return `M${sourceX},${sourceY}
                  L${sourceX},${(sourceY + d.target.y) / 2}
                  L${d.target.x},${(sourceY + d.target.y) / 2}
    ...
    renderLinks(completeLinks, 'complete-link', d => 
      d.source === root || d.target.data.status === 'DONE' ? colors.nodeComplete : colors.link
    );
    ...
    node.each(function (d) {
      ...
      if (isRootNode) {
        ...
        if (d.data.imageComponent) {
          ...
          currentNode
            ...
            .attr('stroke', isComplete ? colors.nodeComplete : colors.nodeIncomplete)
            .attr('stroke-width', 2)
      ...
      currentNode
        .on('mouseover', function (event, d) {
          if (!isRootNode && (d.data.name || d.data.description)) {
            ...
            tooltip
              .html(`
                ${d.data.name ? `<strong>${d.data.name}</strong>` : ''}
    ...
    node
      .append('foreignObject')
      .attr('x', d => -50) // Adjust this value if needed
      .attr('y', d => -60) // Move the label closer to the node
      .attr('width', 100)
  ...
  return <svg ref={svgRef} style={{ background: 'hsl(var(--background))' }}></svg>;
};
