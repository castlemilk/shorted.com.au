...
export function StockSearchField({
  ...
  // Cleanup debounced search on unmount to prevent memory leaks
  useEffect(() => {
    return () => {
...
export function WidgetConfigForm({
  ...
  const renderField = (key: string, fieldSchema: Record<string, unknown>) => {
      ...
      return (
        <div key={key} className="space-y-2">
          ...
          <Input
            ...
            onChange={(e) => onSettingsChange(key, parseInt(e.target.value))}
    ...
    if (
      ...
      return (
        <StockSearchField
          ...
          onStocksChange={(newStocks) => onSettingsChange(key, newStocks)}
        />
      );
    }
                  ...
                  onCheckedChange={(checked) => {
                    if (checked) {
                      ...
                      if (newTypes.length > 0) {
                        onSettingsChange(key, newTypes);
                      } else {
                        const otherOption = availableOptions.find(
                          (o) => o !== option,
                        );
                        if (otherOption) {
                          onSettingsChange(key, [otherOption]);
                        }
                      }
                    }
                  }}
    ...
    if (fieldSchema.type === "object" && key === "stockShortsVisibility") {
      const stocks = (settings?.stocks as string[]) || [];
      const visibility = (value as Record<string, boolean>) || {};
      const dataTypes = (settings?.dataTypes as string[]) || ["shorts"];
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        onSettingsChange("stocks", newStocks);
                        onSettingsChange(key, newVisibility);
                      }}
                    >
                      <X className="h-3 w-3" />
                    </Button>
