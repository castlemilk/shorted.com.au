...
export function StateSuburbExplorer({ stateCode }: StateSuburbExplorerProps) {
  ...
  const suburbs: SuburbDatum[] = useMemo(
    () => (data?.suburbs ?? []).map((s) => ({
      ...
      schoolsGov: s.amenities?.schoolsGov ?? 0, schoolsCatholic: s.amenities?.schoolsCatholic ?? 0,
      schoolsIndependent: s.amenities?.schoolsIndependent ?? 0,
      schoolsPrimary: s.amenities?.schoolsPrimary ?? 0, schoolsSecondary: s.amenities?.schoolsSecondary ?? 0,
      nearestSecondaryKm: s.amenities?.nearestSecondaryKm ?? 0,
