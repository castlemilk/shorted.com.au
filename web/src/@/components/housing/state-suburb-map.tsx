...
import {
  COLUMN_METRIC_GROUPS, HIGHLIGHT_METRICS, METRIC_BY_KEY, METRIC_ICON, amberScale, type MetricKey, type HighlightMetric,
...
// Column metrics are picker sections, in COLUMN_METRIC_GROUPS order.
const COLUMN_SECTIONS = COLUMN_METRIC_GROUPS.map((group) => ({
  ...group,
  metrics: HIGHLIGHT_METRICS.filter((m) => m.kind === "column" && m.group === group.key),
