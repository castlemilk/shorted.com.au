...
export async function generateStaticParams() {
  try {
    ...
    return slugs.slice(0, 52).map((slug) => ({ slug }));
  } catch {
    return [];
  }
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `ASX Short Selling Report: ${weekTitle} | ${siteConfig.name}`;
  const description = `Weekly short selling report for the ASX — ${weekTitle}. Top shorted stocks, biggest movers, and industry analysis from official ASIC data.`;
        ...
        <section>
          ...
          <div className="rounded-lg border border-border/60 overflow-hidden bg-card/50">
            ...
            <div className="divide-y divide-border/40">
              {data.topStocks.slice(0, 20).map((stock, index) => (
                <Link
                  key={stock.code}
