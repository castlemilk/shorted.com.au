...
export function SyncValidationPanel() {
  ...
  const start = useCallback(async () => {
    ...
    const res = await startSyncValidation({ stocks: codes });
      ...
      <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            ...
            for these codes against what is in the database. Nothing is written.
          </p>
        </div>
      </div>
...
function ValidationReportView({
  ...
  return (
    <div className="space-y-3">
      ...
      {stocks && stocks.not_found.length > 0 && (
      ...
      {stocks && stocks.observations.length > 0 ? (
        <Table>
          <TableHeader>
            <TableRow>
        ...
        <p className="text-xs text-muted-foreground">
          No rows for these codes in the window the sync would process.
        </p>
      )}
