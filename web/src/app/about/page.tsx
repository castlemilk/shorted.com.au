...
import {
  getAboutStatistics,
  type AboutPageStatistics,
} from "./actions/get-statistics";
import "./page.css";
...
const Page = () => {
  const [statistics, setStatistics] = useState<AboutPageStatistics | null>(
    null,
  );
  const [isLoadingStats, setIsLoadingStats] = useState(true);

  useEffect(() => {
    // Fetch statistics on client side
    const fetchStats = async () => {
      try {
        setIsLoadingStats(true);
        console.log("Fetching statistics...");
        const stats = await getAboutStatistics();
        console.log("Statistics received:", stats);

        // Always set statistics, even if zeros (so UI can show them)
        setStatistics(stats);

        // Warn if data seems invalid
        if (stats.companyCount === 0 && stats.industryCount === 0) {
          console.warn(
            "Statistics are all zeros - this might indicate a data loading issue",
          );
        }
      } catch (error) {
