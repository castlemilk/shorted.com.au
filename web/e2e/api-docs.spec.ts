...
test.describe("API Documentation", () => {
  ...
  test("should navigate to an endpoint page", async ({ page }) => {
    ...
    if ((await sidebar.count()) === 0) {
      ...
      const menuTrigger = page
        .locator('button:has([class*="Menu"]), button:has-text("Menu")')
        .filter({ visible: true })
        .first();
    ...
    const link = page
      .locator('role=link[name*="RegisterEmail"]')
      .filter({ visible: true })
      .first();
    ...
    await expect(page).toHaveURL(
      /\/docs\/api\/post--register.v1.registerservice-registeremail/,
    );
    await expect(page.locator("h1")).toContainText("RegisterEmail");
    await expect(
      page.locator("text=POST").filter({ visible: true }).first(),
    ).toBeVisible();
  });
  ...
  test("should show code samples", async ({ page }) => {
    ...
    await expect(
      page.locator("text=cURL").filter({ visible: true }).first(),
    ).toBeVisible();
    await expect(
      page.locator("text=JavaScript").filter({ visible: true }).first(),
    ).toBeVisible();
    // Verify syntax highlighting is applied
    await expect(
      page.locator('pre[class*="language-"]').filter({ visible: true }).first(),
    ).toBeVisible();
  });
  ...
  test("should show the Try It panel", async ({ page }) => {
    ...
    const tryItTab = page
      .locator('button:has-text("Try It")')
      .filter({ visible: true })
      .first();
    await tryItTab.click();
    await expect(
      page.locator("text=Test Request").filter({ visible: true }).first(),
    ).toBeVisible();
    await expect(
      page.locator('button:has-text("Send")').filter({ visible: true }).first(),
    ).toBeVisible();
  });
  ...
  test("should search for endpoints via Cmd+K", async ({ page }) => {
    ...
    const dismissBanner = page
      .locator('button[name*="Dismiss"]')
      .filter({ visible: true });
    ...
    const searchButton = page
      .locator('button:has-text("Search docs...")')
      .filter({ visible: true })
      .first();
    ...
    const result = page
      .locator('button:has-text("GetStock")')
      .filter({ visible: true })
      .first();
    ...
    await expect(page).toHaveURL(
      /\/docs\/api\/post--shorts.v1alpha1.shortedstocksservice-getstock/,
    );
  });
});
