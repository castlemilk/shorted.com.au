// Economy iconography — the PROVEN direct-OpenAI generation path.
//
// Calls the OpenAI Images API (gpt-image-1) directly with a transparent
// background, one request per icon, and writes ./out/<id>.png. Modelled on the
// housing-banners direct-OpenAI pattern. Resumable: an icon whose out/<id>.png
// already exists is skipped unless FORCE=1.
//
// Usage:
//   OPENAI_API_KEY=… node web/scripts/economy-icons/generate-icons-openai.mjs
//   ONLY=food,chemicals node .../generate-icons-openai.mjs   # subset
...
async function generateOne(ic, attempt = 1) {
  const prompt = `${ic.subject}. ${STYLE.suffix}`;
